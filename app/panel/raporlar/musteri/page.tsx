...
import { Users, Download, ArrowLeft, TrendingUp, TrendingDown, MapPin, Building, Target, Clock, CheckCircle2, FileText, ChevronRight, X } from "lucide-react"
...
export default function CustomerReportPage() {
        ...
        const fetchData = async () => {
            try {
                ...
                const [custRes, offRes] = await Promise.all([
                    axios.get(`${server}/reports/customers`, {
    ...
    const customerStats = useMemo(() => {
        return customers.map(c => {
            const customerOffers = offers.filter(o => o.musteri?._id === c._id || o.musteri === c._id);
            const totalOffers = customerOffers.length;

            const pendingOffers = customerOffers.filter(o => o.status === 'Gönderildi' || o.status === 'Bekliyor' || o.status === 'Müşteriye Gönderildi' || !o.status)
            const rejectedOffers = customerOffers.filter(o => o.status === 'Reddedildi' || o.status?.includes('Red'))
            const approvedOffers = customerOffers.filter(o => o.status === 'Onaylandı' || o.status?.includes('Onay') || o.status === 'Yönetici Onayladı')
            const deliveredOffers = customerOffers.filter(o => o.status === 'Faturalandı' || o.status === 'Teslim Edildi' || o.status?.includes('Teslim') || o.status?.includes('Kısmi Fatura') || o.status === 'Tamamlandı' || o.status === 'Sevkiyat Halinde')
            ...
            const totalValue = customerOffers.reduce((acc, curr) => acc + (parseFloat(String(curr.urunler?.[0]?.toplamTutar).replace(/[^0-9.-]+/g, "")) || 0), 0)
            const approvedValue = approvedOffers.reduce((acc, curr) => acc + (parseFloat(String(curr.urunler?.[0]?.toplamTutar).replace(/[^0-9.-]+/g, "")) || 0), 0)

            const efficiencyRate = c.efficiencyRate || 0;
            const revenue = customerOffers.reduce((acc, curr) => acc + (parseFloat(String(curr.urunler?.[0]?.toplamTutar).replace(/[^0-9.-]+/g, "")) || 0), 0);
    ...
    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(customerStats.map(c => ({
            "Müşteri": c.company || c.person || "İsimsiz",
            "Şehir": c.city || "-",
            "Oluşturulan Teklif": c.totalOffers,
            "Onaylanan": c.approvedCount,
            "Teslim Edilen": c.deliveredCount,
            "Reddedilen": c.rejectedCount,
            "Toplam Tutar": c.totalValue,
            "Verimlilik(%)": c.efficiencyRate
        })));
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "MusteriKarnesi");
        XLSX.writeFile(workbook, `Musteri_Karnesi_${new Date().toISOString().split('T')[0]}.xlsx`);
    }

    if (selectedCustomer) {
        const stats = customerStats.find(c => c._id === selectedCustomer._id) || selectedCustomer;
        return (
            <div className="flex flex-col gap-6 py-4 font-sans relative">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-semibold text-slate-800  flex items-center gap-2">
                                {stats.company || stats.person} <span className="text-slate-400 font-medium">Karnesi</span>
                            </h1>
                            <div className="flex items-center gap-3 text-[11px] text-slate-500 font-normal mt-1">
                                {stats.city && <span className="flex items-center gap-1"><MapPin className="size-3" /> {stats.city}</span>}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <div className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col justify-between gap-4 relative">
                        ...
                        <div>
                            <p className="text-[11px] font-normal text-slate-400">Tüm Teklifler</p>
                    ...
                    <div className="bg-slate-900 p-5 rounded-[2rem] border border-slate-800 shadow-sm flex flex-col justify-between gap-4 relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <p className="text-[11px] font-normal text-slate-400">Verimlilik</p>
                ...
                <div className="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] overflow-hidden mt-4">
                    <div className="p-6 border-b border-slate-50 flex items-center justify-between">
                        <h3 className="text-sm font-semibold text-slate-800 ">Müşterinin Teklif Geçmişi</h3>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody>
                                {stats.customerOffers.length === 0 ? (
                                    <tr>
                                        <td colSpan={3} className="py-10 text-center">
                                            <p className="text-xs font-normal text-slate-300">Bu müşteriye ait teklif bulunmuyor.</p>
                                        </td>
                                    </tr>
                                ) : (
                                    stats.customerOffers.map((offer: any) => (
        ...
        <div className="flex flex-col gap-6 py-4 font-sans relative">
            ...
            {loading && (
                <div className="absolute inset-0 bg-white/40 backdrop-blur-[1px] z-[100] flex items-center justify-center rounded-[3rem]">
                    <div className="bg-white p-4 rounded-2xl shadow-xl flex items-center gap-3 border border-slate-100">
                        <div className="size-4 bg-purple-500 rounded-full animate-bounce" />
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 px-2">
                <div className="flex items-center gap-3">
                    <div className="size-10 rounded-xl bg-purple-500 text-white flex items-center justify-center shadow-lg shadow-purple-500/20">
                        <Users className="size-5" />
                    </div>
                    <div>
                        <h1 className="text-xl font-semibold text-slate-800 ">Müşteri Karneleri</h1>
                        <p className="text-[11px] text-slate-400 font-normal mt-0.5">Kişi / Firma Bazlı Performans Raporları</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {customerStats.length === 0 && !loading && (
                    <div className="col-span-full py-16 text-center border-2 border-dashed border-slate-200 rounded-[2rem] text-slate-400 font-normal text-sm">
                        Sistemde henüz kayıtlı müşteri bulunamadı.
                    </div>
                )}
                {customerStats.map(customer => (
                    ...
                    >
                        <div className="p-5 flex items-start justify-between border-b border-slate-50">
                            <div className="flex items-center gap-3">
                                <div className="size-12 rounded-2xl bg-slate-50 text-slate-500 flex items-center justify-center font-semibold text-sm border border-slate-100 group-hover:bg-purple-50 group-hover:text-purple-600 group-hover:border-purple-100 transition-colors">
                                    {(customer.company || customer.person || "MS").substring(0, 2).toUpperCase()}
                                </div>
                                <div className="flex flex-col">
                                    <h3 className="text-sm font-semibold text-slate-800  group-hover:text-purple-600 transition-colors line-clamp-1">{customer.company || customer.person}</h3>
                                    <span className="text-[11px] text-slate-400 font-normal mt-0.5 max-w-[150px] truncate">{customer.city || "Belirtilmedi"}</span>
                                </div>
                            </div>
                            <div className="size-8 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center group-hover:bg-purple-50 group-hover:text-purple-600 transition-colors">
                        ...
                        <div className="p-5 flex items-end justify-between gap-4 bg-slate-50/30">
                            <div className="flex flex-col gap-1.5 flex-1">
                                <div className="flex items-center justify-between text-[11px] font-normal">
                                    ...
                                    <span className="text-slate-800">%{customer.efficiencyRate}</span>
                                </div>
                                <p className="text-[10px] text-slate-400 font-normal text-right mb-1.5">{customer.approvedCount} Onay / {customer.totalOffers} Toplam</p>
                                <div className="h-1 w-full bg-gradient-to-r from-rose-500 via-amber-400 to-emerald-500 rounded-full relative">
                                    <div
                                        ...
                                        style={{ left: `${customer.efficiencyRate}%` }}
                                    />
                                </div>
                            </div>
                            <div className="flex flex-col items-end">
                                <span className="text-[10px] font-normal text-slate-400 mb-0.5">Teklif Sayısı</span>
                                <span className="text-xl font-semibold text-slate-800">{customer.totalOffers}</span>
                            </div>
                        </div>
                    </div>
                ))}

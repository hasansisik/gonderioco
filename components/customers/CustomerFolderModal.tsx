...
import { createFolder, updateFolder } from "@/redux/actions/messageActions";
import { cn } from "@/lib/utils";
import toast from "react-hot-toast";

interface CustomerFolderModalProps {
    isOpen: boolean;
    onClose: () => void;
    editingFolder?: any;
}

export const CustomerFolderModal = ({ isOpen, onClose, editingFolder }: CustomerFolderModalProps) => {
    const dispatch = useAppDispatch();
    const { customers } = useAppSelector((state) => state.customer);
    const [name, setName] = useState("");
    const [selectedItems, setSelectedItems] = useState<string[]>([]);
    const [search, setSearch] = useState("");
    const [color, setColor] = useState("#3b82f6"); // Default blue for customers
    ...
    const colors = ["#3b82f6", "#ef4444", "#f97316", "#10b981", "#8b5cf6", "#f59e0b", "#6366f1"];
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-semibold text-slate-800">
                                {editingFolder ? "Klasörü Düzenle" : "Yeni Müşteri Klasörü"}
                            </h2>
                            <p className="text-xs text-slate-400 font-medium">Müşterilerinizi gruplandırın</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 space-y-6">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full rounded-2xl border border-slate-100 bg-slate-50/50 py-3.5 px-5 text-sm focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-medium"
                        />
                    </div>
                    ...
                    <div>
                        <label className="text-[12px] font-normal text-slate-500 block mb-3 px-1 flex justify-between items-center">
                            <span>Müşterileri Seç</span>
                            <span className="text-blue-500">{selectedItems.length} seçili</span>
                        </label>
                        ...
                        <div className="max-h-[200px] overflow-y-auto pr-2 space-y-1 custom-scrollbar">
                            {filteredCustomers.map(customer => (
                                <div
                                    key={customer._id}
                                    onClick={() => toggleItem(customer._id)}
                                    className={cn(
                                        "flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all border",
                                        selectedItems.includes(customer._id)
                                ...
                                >
                                    <div className={cn(
                                        ...
                                        {selectedItems.includes(customer._id) && <Check className="size-3.5" strokeWidth={4} />}
                                    </div>
                ...
                <div className="p-6 bg-slate-50/50 border-t border-slate-100 flex gap-3">
                    ...
                    <button
                        onClick={handleSave}
                        className="flex-2 px-8 py-3.5 rounded-2xl text-sm font-semibold text-white bg-blue-500 shadow-lg shadow-blue-500/20 hover:bg-blue-600 transition-all hover:scale-[1.02] active:scale-95"
                    >
